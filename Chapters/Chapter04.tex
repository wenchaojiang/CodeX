...
\textbf{JH:} Ya okay cuz I think the leak is somewhere near the other one and the army. [referring to building of Territory Army]\ref(Figure \ref{fig:study1ep41})\\
...
The section will present the results of a workshop we conducted with a professional disaster response charity called \acf{RG}. The purpose of these workshop is to get professional feedbacks about the \acf{AO} platform to understand realism of the disaster simulation based on AO. Based on the feedbacks, we reflects on strength, limitations and potential improvements of \ac{AO} platform. \\

The workshop is centred on a demonstration of \ac{HAC-ER} system for disaster response (DR) \cite{Ramchurn2015}. The AO is a component of the \ac{HACS} demo system. Others components include information crowdsourcing, multi-UAV control interface for DR, and DR operation provenance store (Section \ref{sec:lraisupport}). Each component are allocated one session (about 45 mins). The \ac{AO} session is structured as follows:\\

\begin{enumerate}
	\item A presentation introducing the \ac{AO} platform and HQ interface.
	\item A handles-on session for \ac{RG} members to operate the HQ interface. For the purpose of demo, the field players in \ac{AO} are simulated.
	\item A discussion of \ac{AO} platform for Rescue Global to reflect on some themes including: Command and Control structure, Division of Labour/Task and team planning.
\end{enumerate} 

We also encourages free discussions throughout the \ac{AO} session, which leads to some emerged themes including: Data connection, time-scale of planning in real DR operations and other interface improvement suggestions.  

\subsection{The Rescue Global feedbacks}
Based the feedbacks from the \ac{RG}, the \ac{AO} setting  mirrors some aspects of the RG`s disaster operations. Firstly, the \ac{RG} member quickly recognise the task-team matching mechanism is very similar to the aspects of resource-needs matching in their operation. At the beginning of the introduction presentation of\ac{AO}, one of the responder commented: \\
...
For feedbacks, we also noticed that the time-scale of the \ac{RG}'s operation is much longer then the AO. As one RG member describe how tasks are allocated in a multi-agency operations: 
...
Although the time-scale of \ac{AO} is unrealistic for \ac{RG} time,  they think the \ac{AO} can be seen as a ideal, speed-up version of mulit-agency operation, because it automates lots of manual process in terms of information capturing and presentation and promote commonly recognised information picture across agencies:
...
In the previous section, we have reflected on both realistic and unrealistic aspects of \ac{AO} setting based on the feedbacks from \ac{RG} workshop. The resource target mapping and teaming aspects of \ac{AO} setting are found to be similar to activities in the \ac{DR} operations, though the teaming in DR operations are likely to be conducted in an informal way. On the other hand, both time scale and command structure in the \ac{AO} setting do not match what happens in the \ac{DR} operations. The time scale of \ac{RG}`s operation is much longer then the \ac{AO}. And Command and Control framework in DR has hierarchical levels and sectorisation for some levels, both of which are missing in the \ac{AO} setting. Further, the assumption of good data connections in the field are also fund unrealistic. However the \ac{RG} team thought the core planning support aspects of \ac{AO} would still be useful even when data connection is poor. \\

Although \ac{AO} are not built as a tool to support real disaster operation (It is designed to be a simulation game), the \ac{RG} team still found some aspects of \ac{AO} demonstrates elements of future planning support system desirable to them. Firstly, the \ac{AO} portrayed a picture of automated information capturing and visualisation which is appreciated by \ac{RG}. Secondly, \ac{RG} also appreciate that the division of labour facilitated by the \ac{AO} setting, in which the HQ (with the planning agent) handles the task planning while the field responders just need to ``gather information, file it back, and forget it''\\

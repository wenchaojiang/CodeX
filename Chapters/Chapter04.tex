...
\subsection{The Rescue Globalfeedbacks}
...
\section{Summary}
The objective of the research presented here was to explore the design surrounding an In-the-loop interaction. The exploration is conducted through field trial of an AtomicOrchid(AO) probe with In-the-loop support, with particular focus in the planning activities in control room. The Game probe is evolved from the On-the-loop version of AO that is trialled in study 2 (see ref). The game probe allows xxxxxx. 

Findings from interaction analysis of field observations, triangulated with log files, reveal how the In-the-loop interactions played out. [Spell it out!]. A set of interactional issues also emerged and discussed in section ref: . 

The interactional issues discovered also lead to the design implications which can be grouped into three themes: Common ground


In addition, a workshop with professional organisation has been reported to . The feedback reveals strength and weakness of xxxxx AO simulation: 

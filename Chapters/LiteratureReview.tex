...
%*****************************************
The overarching background of the task planning activities in disaster management. This thesis particularly focuses on the disaster response which is a period of disaster response requiring high level of real-tiem coordination and capability to handle uncertainties from disaster environment. \\

Computational support for planning has long be studied .  There is not real ones that support real time planing in DR period, though similarities can . ICT supports are there which forms a bases for more task planning support. AI technologies are there in the labs but most of them have not been applied to real world. \\

Automation and its issues have long been studied by the researchers, which warned us that there could be significant difficulty for applying tech support to human teams. We believe so. Therefore, we will try to fill the gap of not only the application area but also social and technical. \\

There is not straightforward way to study the system with two exsiting gap. It is hard. However the serious game has inspired the approach that we will use. \\


1. What is Disaster Response
4. two prospects automation and CSCW can go together 
3. key area tech support, gap I am trying to tackle, but I am not do it in real.
5. Therefore approach.
...
\section{Technological support for task planning}
Planning support systems is unlikely to be an isolated computational entities. It is better to be seen as `toolbox' that contains various connected components \cite{Wattegama2012}. To give an overview of current technological practices related planning support, This section will review three related research domains in the context of computational DR support, that is -  Task planning systems for plan formulation and evaluation ; the Information and communication technologies (ICT) for information acquisition and management; and the AI(Artificial intelligence) based technologies for disaster simulation, and task optimisation.  \\
...
So far, we have framed the problem of task planning in DR in section \ref{sec:lrplanning} and reviewed three research domains related to development of DR planning system in this section. It should be emphasised that this PhD work is primarily interested in a real-time task planning system that utilise ICT technologies as its underlying infrastructures and apply intelligent coordination algorithms for plan generation. This kind of system can be located in the overlapping area of the three research domains that is reviewed in this section (see figure \ref{fig:SystemFraming}).\\
